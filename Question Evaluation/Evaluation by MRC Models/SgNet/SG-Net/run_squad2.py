...
def read_squad_examples(input_file, input_tag_file, output_dir, is_training=False):
    ...
    try:
        output_prediction_file = os.path.join(output_dir, "epoch_2_predictions.json")
        with open(output_prediction_file, 'r') as file:
            final = json.load(file)
            blacklist = list(final.keys()) 
        print("LENGTH")
        print(len(blacklist))
        df = df.loc[~df['qid'].isin(blacklist)]
        input_tag_data = [item for item in input_tag_data if item['qas_id'] not in blacklist]
        print("LENGTH")
        print(len(input_tag_data))
    ...
    for idx, tag_data in enumerate(tqdm(input_tag_data,ncols=50)):
        ...
        dqtag_data = {
            ...
            "span_doc": [eval(i) for i in tag_rep["hpsg_list_doc"]],# for i in sen_span],
            "type_doc": [i for sen in tag_rep["pred_type_doc"] for i in sen],
            "head_doc": [int(i) for i in tag_rep["pred_head_doc"]],# for i in sen_head],
    ...
    examples = []
    for index, row in tqdm(df.iterrows(), total=df.shape[0],ncols=50):
        paragraph_text = row["context"]
        ...
        for sent_tokens in new_sen_list:
            ...
            for _ in sent_tokens:
                try:
                    ...
                    new_span_sen.append((0,0))
                try:
                    new_head_sen.append(head_doc[cnt])
                except:
                    new_head_sen.append((0,0))
                try:
                    new_type_sen.append(type_doc[cnt])
                except:
                    new_type_sen.append((0,0))
        ...
        question_text = row["modified question"]
        start_position = None
        end_position = None
        orig_answer_text = None
        is_impossible = False
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(tqdm(examples,ncols=50)):
        ...
        for sen_idx, sen_span in enumerate(all_doc_span):
            ...
            for idx, (start_ix, end_ix) in enumerate(sen_span):
                if sen_offset + start_ix - 1 < 0 or sen_offset + end_ix - 1 < 0:
                    #print(f"Skipping invalid indices: start_ix={start_ix}, end_ix={end_ix}")
                    continue
                
                head_start, head_end = doc_org_to_split_map[sen_offset + idx]

                # Ensure start_ix and end_ix are valid
                if sen_offset + start_ix - 1 not in doc_org_to_split_map or sen_offset + end_ix - 1 not in doc_org_to_split_map:
                    #print(f"Skipping invalid map access: start_ix={start_ix}, end_ix={end_ix}")
                    continue
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            doc_subarray_shape = doc_span_mask[start_doc_ix:end_doc_ix + 1, start_doc_ix:end_doc_ix + 1].shape

            # 0 count for [CLS] and select_doc_len+1 count for [SEP]
            if doc_subarray_shape == (select_doc_len + 1, select_doc_len + 1):
                input_span_mask[1:select_doc_len + 2, 1:select_doc_len + 2] = doc_span_mask[start_doc_ix:end_doc_ix + 1,
                                                                              start_doc_ix:end_doc_ix + 1]

            #print(que_span_mask)
            if input_span_mask[select_doc_len + 2:select_doc_len + select_que_len + 2, select_doc_len + 2:select_doc_len + select_que_len + 2].shape == que_span_mask.shape:
                input_span_mask[select_doc_len + 2:select_doc_len + select_que_len + 2,
                                select_doc_len + 2:select_que_len + select_doc_len + 2] = que_span_mask
            else:
                #print("Shape mismatch. Adjusting the shapes to match.")
                # Resize que_span_mask if necessary
                que_span_mask_resized = np.resize(que_span_mask, (select_que_len, select_que_len))
                input_span_mask[select_doc_len + 2:select_doc_len + select_que_len + 2,
                                select_doc_len + 2:select_que_len + select_doc_len + 2] = que_span_mask_resized
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        try:
            ...
            val = all_predictions[example.qas_id]
            all_predictions[example.qas_id] = []
            all_predictions[example.qas_id].append(val)
            all_predictions[example.qas_id].append(nbest_json[0]["text"])
        ...
        try:
            ...
            with open(output_prediction_file, "w") as writer:
                writer.write(json.dumps(existing_data, indent=4))
...
def main():
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        eval_examples = read_squad_examples(
            input_file=args.predict_file, input_tag_file=args.predict_tag_file, is_training=False, output_dir= args.output_dir)
        try:
